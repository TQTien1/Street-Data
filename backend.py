...
uploaded_df = pd.read_csv("big_table_2.csv")
traffic_coords = pd.concat([uploaded_df[['s_lat', 's_long','dbscan_cluster_labels']].rename(columns={'s_lat': 'lat', 's_long': 'long'}), 
                        uploaded_df[['e_lat', 'e_long','dbscan_cluster_labels']].rename(columns={'e_lat': 'lat', 'e_long': 'long'})], 
                        axis=0).drop_duplicates().reset_index(drop=True)
unique_street_names = uploaded_df['street_name'].unique().tolist()

def heuristic(node, goal):
    node_coords = uploaded_df[uploaded_df['dbscan_cluster_labels'] == node][['s_lat', 's_long']].mean()
    goal_coords = uploaded_df[uploaded_df['dbscan_cluster_labels'] == goal][['s_lat', 's_long']].mean()
...
def get_neighbors(node, cluster_weights, distance_threshold=0.03):
    ...
    for cluster in cluster_weights.keys():
        if cluster != node:
            cluster_coords = uploaded_df[uploaded_df['dbscan_cluster_labels'] == cluster][['s_lat', 's_long']].mean()
...
def a_star(start, goal):
    cluster_weights = uploaded_df[uploaded_df['slow_traffic'] == 1].groupby('dbscan_cluster_labels').size().to_dict()
    open_set = []
...
def create_map(traffic_coords, optimized_path):
    ...
    for i in range(len(optimized_path) - 1):
        ...
        folium.RegularPolygonMarker(
            ...
            number_of_sides=3,
            radius=6,
            rotation=0,  # Rotate the arrowhead to point in the correct direction
            color="red",
            fill=True,
            fill_color="red"
        ).add_to(map_folium)
...
# Create a folium map to visualize all data points
...
if st.sidebar.button("Submit",key="2"):
    if start and end in [name.upper() for name in unique_street_names]:
        start = uploaded_df[uploaded_df['street_name'].str.upper() == start]['dbscan_cluster_labels'].values[0]
        end = uploaded_df[uploaded_df['street_name'].str.upper() == end]['dbscan_cluster_labels'].values[0]
